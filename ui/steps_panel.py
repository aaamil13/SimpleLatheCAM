...
Each ToolSequence is shown as a collapsible section header.
Each operation inside is a StepCard with:
  - enable/disable checkbox
  - name + param summary label
  - gear button → opens OperationEditDialog (modal, ParamsPanel inside)
  - delete button
...
class OperationEditDialog(QDialog):
    """Modal dialog wrapping ParamsPanel for a single operation."""
...
class StepCard(QFrame):
    selected = Signal(int, int)   # seq_idx, op_idx
    ...
    def __init__(
        ...
        short  = "  ".join(
            f"{k}={v:.2g}" for k, v in list(op.params.items())[:2]
        )
    ...
    def _open_edit(self) -> None:
        ...
        dlg = OperationEditDialog(
            self._model, self._seq_idx, self._op_idx, self
        )
        dlg.exec()
    ...
    def _rebuild(self) -> None:
        ...
        for si, seq in enumerate(self._model.recipe.tool_sequences):
            ...
            for oi, op in enumerate(seq.operations):
                card = StepCard(self._model, si, oi, op, self._inner)
        ...
        self._highlight(
            self._model.selected_seq,
            self._model.selected_op,
        )
